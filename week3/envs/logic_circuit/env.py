...
class LogicCircuitEnv(Env):
    ...
    def generate(
        ...
        **kwargs,
        ...
        id_prefix = kwargs.pop("id_prefix", "sample")
        ...
        for i in range(int(num_of_questions)):
            ...
            sample_id = f"{id_prefix}_d{difficulty}_{seed}_{i}"
            items.append(Data(id=sample_id, question=question, answer=str(y), difficulty=difficulty, metadata=metadata))
    ...
    def _sample_instance(self, rng: random.Random, config: Dict[str, Any], max_attempts: int):
